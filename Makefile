...
FOCUSED_CRATES := use-document-store use-key-value-store use-wide-column use-graph-store use-cache-store use-search-index use-vector-store use-timeseries-store use-document-path use-partition-key use-consistency use-change-event
FACADE_CRATE := use-nosql
